...
export function Sidebar({ activeTab, setActiveTab, onLogout, user, isOpen, setIsOpen }: SidebarProps) {
  const navItems = [
    ...
    { id: 'shifts', label: 'Fechamento de Caixa', icon: LockIcon },
  ...
  return (
    <>
      ...
      <nav className="flex-1 min-h-0 px-4 space-y-1 mt-4 overflow-y-auto custom-scrollbar">
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center space-x-3 px-4 py-2.5 rounded-xl transition-colors border ${
                isActive 
